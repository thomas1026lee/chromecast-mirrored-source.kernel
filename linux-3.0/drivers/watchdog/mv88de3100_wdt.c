...
static irqreturn_t mv88de3100_wdt_irq(int irqno, void *dev_id)
{
	struct mv88de3100_wdt *wdt = (struct mv88de3100_wdt *)dev_id;

	/* clear interrupt */
	rdlp(wdt, WDT_EOI);
	dev_warn(wdt->dev, "watchdog timer expired\n");
	mv88de3100_wdt_keepalive(wdt);
...
static int __devinit mv88de3100_wdt_probe(struct platform_device *pdev)
{
	...
	wdt->reg_base = ioremap(wdt->mem->start, size);
	...
	wdt->irq = platform_get_resource(pdev, IORESOURCE_IRQ, 0);
	if (wdt->irq == NULL) {
		dev_err(wdt->dev, "no irq resource specified\n");
		ret = -ENOENT;
		goto err_map;
	}

	ret = request_irq(wdt->irq->start, mv88de3100_wdt_irq,
			0, pdev->name, wdt);
	if (ret != 0) {
		dev_err(wdt->dev, "failed to install irq (%d)\n", ret);
		goto err_map;
	}
