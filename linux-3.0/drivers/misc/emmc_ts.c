		       ...
		       const void *buf, size_t size)
{
	...
	printk(KERN_INFO DRV_NAME ": write off=0x%08llx size=0x%x\n", off, size);
...
static int get_fts_dev_node(void)
{
	...
	ret = filp->f_pos;
	printk(KERN_INFO DRV_NAME ": %s partition is on %s, 0x%x bytes\n",
	       CONFIG_EMMC_TS_PARTITION, fts_dev_node, ret);
...
static int flash_ts_open(struct inode *inode, struct file *file)
{
	...
	printk(KERN_INFO DRV_NAME ": chunk: 0x%x\n", ts->chunk);
...
const struct file_operations flash_ts_proc_fops = {
	.owner = THIS_MODULE,
...
static int __init flash_ts_init(void)
{
	...
	proc_entry = create_proc_entry(DRV_NAME, 0, NULL);
	if (proc_entry)
		proc_entry->proc_fops = &flash_ts_proc_fops;
