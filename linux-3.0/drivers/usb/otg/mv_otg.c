...
static int mv_otg_set_host(struct otg_transceiver *otg_p, struct usb_bus *host)
{
	struct mv_otg *mvotg = container_of(otg_p, struct mv_otg, otg);

	if (host) {
		if ((otg_p->state == OTG_STATE_A_HOST) || (otg_p->state == OTG_STATE_A_WAIT_BCON)) {
			/* stop old host first */
			if (otg_p->host) {
				mv_otg_start_host(mvotg, 0);
			}

			/* start the new host right away*/
			otg_p->host = host;
...
static int mv_otg_set_peripheral(struct otg_transceiver *otg_p,
					struct usb_gadget *gadget)
{
	...
	/* start the gadget right away if otg state is OTG_STATE_B_PERIPHERAL */
	dev_dbg(&mvotg->dev->dev, "ID pin=%d, otg state %d\n", mvotg->otg_ctrl.id, otg_p->state);
...
static void mv_otg_update_inputs(struct mv_otg *mvotg)
{
	...
	/* because mv HW limitation, set vbus vld to 1 */
