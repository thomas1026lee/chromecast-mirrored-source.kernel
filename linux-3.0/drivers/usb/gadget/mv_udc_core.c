...
static void udc_stop(struct mv_udc *udc)
{
	...
	tmp &= ~USBCMD_RUN_STOP;
	writel(tmp, &udc->op_regs->usbcmd);
	dev_info(&udc->dev->dev, "udc_stopped !!!\n");
}
...
static void udc_start(struct mv_udc *udc)
{
	...
	/* AHB burst INCR16 */
	writel(0x07, ((u32)(udc->base_regs)) + USB_BASE_SUBS_CFG);
...
static int udc_reset(struct mv_udc *udc)
{
	...
	/* disable double priming on both RX and TX for low bandwidth */
	tmp |= USBMODE_STREAM_DISABLE;
	...
	writel((EPCTRL_TX_DATA_TOGGLE_RST | EPCTRL_RX_DATA_TOGGLE_RST), &udc->op_regs->epctrlx[0]);
...
static int mv_udc_enable(struct mv_udc *udc)
{
	if(udc->active)
		return 0;
	udc->active = 1;
	return mv_udc_phy_init(udc->phy_regs);
}

static int mv_udc_disable(struct mv_udc *udc)
{
	udc->active = 0;
	return 0;
}
...
static int mv_udc_vbus_session(struct usb_gadget *gadget, int is_active)
{
	...
	if (udc->vbus_active) {
		...
		//TODO: power notification not implement yet
		udc->power = 0;
		...
		int (*bind)(struct usb_gadget *))
{
	...
	if (retval) {
		dev_err(&udc->dev->dev, "bind to driver %s failed, Error %d\n",
...
int mv_udc_probe(struct platform_device *dev)
{
	...
	r = platform_get_resource_byname(udc->dev, IORESOURCE_MEM, "baseregs");
	if (r == NULL) {
	...
	udc->base_regs = (void __iomem *)ioremap(r->start, resource_size(r));
	if (udc->base_regs == NULL) {
	...
	udc->cap_regs = (struct mv_cap_regs __iomem *)ioremap(r->start, resource_size(r));
	...
	printk("udc_probe transceiver %p \n", udc->transceiver);
	if (udc->transceiver) {
		/* stop all the transfer in queue*/
		stop_activity(udc, udc->driver);
		udc_stop(udc);
