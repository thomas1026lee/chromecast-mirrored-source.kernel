...
static __init char *get_cmdline(struct tag *tag)
{
    char *c = NULL;
    while (tag->hdr.size) {
        if (tag->hdr.tag == ATAG_CMDLINE) {
            c = tag->u.cmdline.cmdline;
            break;
        }
        tag = tag_next(tag);
    }
 ...
 */
void __init mv88de31xx_android_fixup(struct tag *tag,
                                     char **from)
{
    ...
    cmdline = get_cmdline(tag);
    if (!cmdline){
        printk(KERN_ERR "Cannot find CMDLINE in tags @ 0x%08X",
                        (unsigned int)tag);
        return;
    }
