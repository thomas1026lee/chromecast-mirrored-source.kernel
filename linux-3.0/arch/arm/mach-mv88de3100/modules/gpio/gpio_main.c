...
#define IRQ_APB_SMGPIOINST_NUM(port) (port >= 8 ? IRQ_SM_GPIO1 : IRQ_SM_GPIO0)
		...
		unsigned int cmd, unsigned long arg)
{
	...
	switch(cmd) {
		...
		case GPIO_IOCTL_INIT_IRQ:
			...
			ret = request_irq(IRQ_SM_GPIO1, gpio_interrupt_handle, IRQF_SHARED,
					GALOIS_GPIO_NAME, &gpio_dev_id);
			if (ret) {
				free_irq(IRQ_APB_GPIOINST0, &gpio_dev_id);
				free_irq(IRQ_APB_GPIOINST1, &gpio_dev_id);
			...
			ret = request_irq(IRQ_SM_GPIO0, gpio_interrupt_handle, IRQF_SHARED,
					GALOIS_GPIO_NAME, &gpio_dev_id);
			if (ret) {
				free_irq(IRQ_APB_GPIOINST0, &gpio_dev_id);
				free_irq(IRQ_APB_GPIOINST1, &gpio_dev_id);
				free_irq(IRQ_APB_GPIOINST2, &gpio_dev_id);
				free_irq(IRQ_APB_GPIOINST3, &gpio_dev_id);
				free_irq(IRQ_SM_GPIO1, &gpio_dev_id);
				printk("error: failed to request_irq %d.\n", IRQ_SM_GPIO0);
				goto init_irq_exit;
			}
			irq_init++;
...
init_irq_exit:
		...
		case GPIO_IOCTL_EXIT_IRQ:
			...
			free_irq(IRQ_APB_GPIOINST3, &gpio_dev_id);
			free_irq(IRQ_SM_GPIO1, &gpio_dev_id);
			free_irq(IRQ_SM_GPIO0, &gpio_dev_id);
			mutex_unlock(&irq_init_mutex);
			break;
		default:
			return -EPERM;
...
#ifdef CONFIG_PM
static galois_gpio_reg_t g_gpio_reg;
static int gpio_suspend(struct device *dev)
{
	gpio_get_info(&g_gpio_reg);
...
static int gpio_resume(struct device *dev)
{
	...
	for (i = 0; i < GPIO_PORT_NUM; i++) {
		if (gpio_irq_mode & (1 << i)) {
			...
			printk("Resume GPIO setting: enable IRQ for port %d\n", i);
		}
	}
	...
	for (i = 0; i < GPIO_PORT_NUM; i++) {
		...
		int data = g_gpio_reg.port_dr & (1<<i);
		...
		printk("Resume GPIO setting: port %d, mode %d", i, mode);
...
static struct platform_driver galois_gpio_drv = {
	.driver = {
		.name	= "mv88de3100_gpio",
		.owner	= THIS_MODULE,
#ifdef CONFIG_PM
		.pm	= &gpio_pm_ops,
#endif
...
static struct platform_device galois_gpio_dev = {
	.name = "mv88de3100_gpio",
};

static int __init gpio_dummy_probe(struct platform_device *dev)
{
	if (dev == &galois_gpio_dev)
		return 0;
	return -ENODEV;
}
...
static int __init galois_gpio_init(void)
{
	...
	platform_device_register(&galois_gpio_dev);
	platform_driver_probe(&galois_gpio_drv, gpio_dummy_probe);
...
static void __exit galois_gpio_exit(void)
{
	...
	platform_device_unregister(&galois_gpio_dev);
	platform_driver_unregister(&galois_gpio_drv);
}
