...
#else
#define I2C_SYS_CLOCK		75
#define I2C_SM_CLOCK		20
#endif
...
#define I2C_RegRead(a, pv)	(*pv = __raw_readl(base_addr+(a)))
#define I2C_RegWrite(a, v)	__raw_writel(v, base_addr+(a))
...
int i2c_master_set_speed(int master_id, int type, int speed)
{
	...
	if (master_id >= 2)
		sysClk = I2C_SM_CLOCK * 12 / 10;
	else
		sysClk = I2C_SYS_CLOCK * 12 / 10;
	...
	if (type == I2C_STANDARD_SPEED) {
		...
		case 100: /* standard-speed */
			...
			I2C_RegWrite(I2C_REG_SS_SCL_HCNT, 4000*sysClk/1000); /* 4000-ns minimum HIGH-period*/
			I2C_RegWrite(I2C_REG_SS_SCL_LCNT, 4700*sysClk/1000); /* 4700-ns minimum LOW-period*/
			break;
		case 400: /* fast-speed */
			I2C_RegRead(I2C_REG_CON, &i);
			i &= ~(3<<CON_SPEED);
			...
			I2C_RegWrite(I2C_REG_FS_SCL_HCNT, 600*sysClk/1000); /* 600-ns minimum HIGH-period*/
			I2C_RegWrite(I2C_REG_FS_SCL_LCNT, 1300*sysClk/1000); /* 1300-ns minimum LOW-period*/
			break;
		case 3400: // high-speed
			...
			I2C_RegWrite(I2C_REG_HS_SCL_HCNT, 60*sysClk/1000); /* 60-ns minimum HIGH-period for 100pF loading, and 120-ns for 400pF loading. */
			I2C_RegWrite(I2C_REG_HS_SCL_LCNT, 160*sysClk/1000); /* 160-ns minimum LOW-period for 100pF loading, and 320-ns for 400pF loading. */
			break;
		default:
...
int i2c_master_init(int master_id)
{
	...
	// set default I2C master speed: 300KHz fast-speed mode, the measured frequency is 380KHz
	i2c_master_set_speed(master_id, I2C_CUSTOM_SPEED, 300);
...
int i2c_master_writeread_bytes(int master_id, int slave_addr, int addr_type, unsigned char *pwbuf, int wnum, unsigned char *prbuf, int rnum, int dum )
{
	...
	if ((pwbuf && wnum > 0) && (prbuf && rnum > 0)) {
		...
		/* write  transaction */
		if (wait_event_interruptible(i2c_wait, write_done[master_id])) {
			I2C_RegWrite(I2C_REG_ENABLE, ~EN_ENABLE);
			return -ERESTARTSYS;
		}
	} else if (prbuf && rnum > 0) {
		/* read transaction */
		if (wait_event_interruptible(i2c_wait, read_done[master_id])) {
			I2C_RegWrite(I2C_REG_ENABLE, ~EN_ENABLE);
			return -ERESTARTSYS;
		}
	} else {
