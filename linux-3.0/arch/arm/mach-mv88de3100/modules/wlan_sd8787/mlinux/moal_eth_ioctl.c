 ...
 */
int
woal_setget_priv_macctrl(moal_private * priv, t_u8 * respbuf, t_u32 respbuflen)
{
    int data = 0;
    mlan_ioctl_req *req = NULL;
    mlan_ds_misc_cfg *cfg = NULL;
...
#define CMD_BUF_LEN   2048
    ...
    if (strncmp(buf, CMD_MARVELL, strlen(CMD_MARVELL)) == 0) {
        ...
        if (strnicmp
            ...
            /* Hotspot CFG */
            len = woal_priv_hotspotcfg(priv, buf, priv_cmd.total_len);
            goto handled;
        } else
            if (strnicmp
            ...
            if (strnicmp
                (buf + strlen(CMD_MARVELL), PRIV_CMD_MAC_CTRL,
                 strlen(PRIV_CMD_QCONFIG)) == 0) {
            /* Queue config */
            len = woal_setget_priv_macctrl(priv, buf, priv_cmd.total_len);
            goto handled;
        } else {
