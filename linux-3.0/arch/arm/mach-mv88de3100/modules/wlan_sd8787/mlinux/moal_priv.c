...
woal_associate_ssid_bssid(moal_private * priv, struct iwreq *wrq)
{
    ...
    mac_idx = 0;
    buflen = MIN(wrq->u.data.length, sizeof(buf));
    ...
    strncpy(ssid_bssid.ssid.ssid, buf + i, sizeof(ssid_bssid.ssid.ssid));

    PRINTM(MCMND,
           "iwpriv assoc: AP=[%02x:%02x:%02x:%02x:%02x:%02x], ssid(%d)=[%s]\n",
           ssid_bssid.bssid[0], ssid_bssid.bssid[1], ssid_bssid.bssid[2],
           ssid_bssid.bssid[3], ssid_bssid.bssid[4], ssid_bssid.bssid[5],
...
woal_get_key_ioctl(moal_private * priv, struct iwreq *wrq)
{
    ...
    wrq->u.data.length = sizeof(key_ascii) + 1;
    if (wrq->u.data.pointer) {
        if (copy_to_user(wrq->u.data.pointer, &key_ascii, tmp - key_ascii)) {
            PRINTM(MERROR, "copy_to_user failed\n");
            ret = -EFAULT;
            goto done;
        }
