...
{
	int n;
	mlan_memcpy((void *) &n, (void *) pBuf, 4);
	return n;
}

static int
woal_debug_read_histogram(char *page, char **s, off_t off, int cnt, int *eof, void *data)
{
	int val = 0;
	int ix, src_ix;
	unsigned int i;
	...
	char *pHistDataBuf;
	unsigned int numHistSamples = 0;

	int nSizeHistData = (RX_RATE_MAX+SNR_MAX+NOISE_FLR_MAX+SIG_STRENGTH_MAX)*4;

	ENTER();

	pHistDataBuf = (char *) kmalloc(nSizeHistData, GFP_KERNEL);
	for (ix = 0; ix < nSizeHistData; ix++)
		pHistDataBuf[ix] = 0;
	...
	if (0 != mlan_hist_data_get(pHistDataBuf, &numHistSamples))
	{
		kfree(pHistDataBuf);
	        LEAVE();
	        return MLAN_STATUS_FAILURE;
	...
	p += sprintf(p, "total samples = %d\n", numHistSamples);
	p += sprintf(p, "rx rates (in Mbps): 0=1M   1=2M   2=5.5M  3=11M   4=6M  5=9M  6=12M\n");
	p += sprintf(p, "                    7=18M  8=24M  9=36M  10=48M  11=54M   12-19 == MCS0-7(BW20)\n\n");

	src_ix = 0;

	for (ix = 0; ix < RX_RATE_MAX; ix++){
		i = woal_debug_get_uint((char *) (src_ix + pHistDataBuf));
		src_ix += 4;
		if (i) {
...
static int
woal_debug_write_histogram(struct file *f, const char *buf, unsigned long cnt, void *data)
{
...
woal_debug_entry(moal_private * priv)
{
    ...
    r2->read_proc = woal_debug_read_histogram;
    r2->write_proc = woal_debug_write_histogram;
#if LINUX_VERSION_CODE < KERNEL_VERSION(2,6,30)
