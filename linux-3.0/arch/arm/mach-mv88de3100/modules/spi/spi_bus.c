...
#define MAX_CMD_COUNT 256 /* in byte */
...
static ssize_t process_spi_cmd(galois_spi_info_t *spi_info, galois_spi_cmd_t *spi_cmd, int wr)
{
	...
	if ((spi_cmd->count > MAX_CMD_COUNT) || (spi_cmd->count < 1)) {
...
static long galois_spi_ioctl(struct file *file,
...
static struct file_operations galois_spi_fops = {
	...
	.unlocked_ioctl		= galois_spi_ioctl,
};
...
static int spi_resume(struct device *dev)
{
	...
	for (i = 0; i < GALOIS_SPIBUS_NUM; i++) {
		...
		if (usable) {
			...
			printk("Restore SPI master [%d], slave [%d]\n", mst_id, slave_id);
...
static int __init spi_dummy_probe(struct platform_device *dev)
{
	if (dev == &spi_dev)
		return 0;
	return -ENODEV;
...
static int __init galois_spi_init(void)
{
	...
	platform_driver_probe(&spi_drv, spi_dummy_probe);
#endif

	return 0;
}

static void __exit galois_spi_exit(void)
{
	int i;
...
module_init(galois_spi_init);
module_exit(galois_spi_exit);
