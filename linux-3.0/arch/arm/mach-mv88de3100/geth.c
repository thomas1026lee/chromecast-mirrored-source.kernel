...
static void phy_init(struct geth_private *priv)
{
	...
	value = __raw_readl(0xF7FE1404);
	value &= ~(0x1);
	__raw_writel(value, 0xF7FE1404);
#endif
	...
	mvEthPhyWrite(priv, priv->mii.phy_id, 0x1E, 0x11C);//Set ADC bias current
...
#if !defined(CONFIG_MV88DE3100_BG2_Z1) && !defined(CONFIG_MV88DE3100_BG2_Z2)
	mvEthPhyWrite(priv, priv->mii.phy_id, 0x1D, 8);
	mvEthPhyWrite(priv, priv->mii.phy_id, 0x1E, 0xE24D);
#else
	//set negoation mode
	mvEthPhyWrite(priv, priv->mii.phy_id, 0x1C, 0xC03);
#endif
	return;
#endif
...
static int geth_open(struct net_device *dev)
{
	...
	err = request_irq(dev->irq, geth_irq,
...
static const struct net_device_ops geth_netdev_ops = {
	...
	.ndo_set_multicast_list	= geth_set_multicast_list,
...
static int __devexit geth_remove(struct platform_device *pdev)
{
...
static struct platform_driver geth_driver = {
	.probe		= geth_probe,
	.remove		= __devexit_p(geth_remove),
...
static void __exit geth_cleanup_module(void)
{
	platform_driver_unregister(&geth_driver);
}
module_exit(geth_cleanup_module);

MODULE_AUTHOR("Jisheng Zhang  <jszhang@marvell.com>");
MODULE_DESCRIPTION("Ethernet driver for Marvell MV88DE Series SoC Ethernet");
MODULE_LICENSE("GPL");
